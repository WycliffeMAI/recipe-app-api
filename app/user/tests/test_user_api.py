...
class PublicUserApiTests(TestCase):
    ...
    def test_create_token_for_user(self):
        """test generator token for valid credetials"""
        user_details = {
            "name": "Test Name",
            "email": "test@example.com",
            "password": "Viv773oklife"
        }
...
class PrivateUserAPITests(TestCase):
    ...
    def test_postme_not_allowed(self):
        """Test not allowed for the url"""
        res = self.client.post(ME_URL, {})

        self.assertEqual(res.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)

    def test_update_user_profile(self):
        """Test updating the user profile for the authenticatded."""
        payload = {"name": "updated name", "password": "newpassword"}

        res = self.client.patch(ME_URL, payload)

        self.user.refresh_fromdb()
